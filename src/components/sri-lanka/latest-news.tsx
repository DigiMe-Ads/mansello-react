        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-slate-500">
              Tips, guides and stories from Colombo, Negombo and beyond.
            </p>
          </div>
        ...
        {posts.length > 0 && (
          <div className={`mt-10 grid gap-6 ${miniPosts.length > 0 ? "lg:grid-cols-[1fr_400px]" : ""}`}>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              {miniPosts.map((post) => (
                <Link key={post.id} href={`/sri-lanka/blog/${post.slug}`} className="flex items-stretch gap-3">
            ...
            {featured && (
              <Link
                href={`/sri-lanka/blog/${featured.slug}`}
